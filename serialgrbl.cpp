...
string rstrip(string input){            // REALIZAR LA FUNCION DE QUITAR LOS CARACTERES BLANCOS DEL FINAL
    string salida;
    return salida;
}
...
string lstrip(string input, string caracter){            // REALIZAR LA FUNCION DE QUITAR LOS CARACTERES BLANCOS DEL FINAL
    string salida;
    return salida;
}
...
void SerialGRBL::Update(){
    ...
    while(true){ // poner la condicion _stop_event.is_set() en base al evento
        ...
        vector<string> temp_decode = partition(read_buffer, "\n");
        ...
        if(temp_decode[1] == "\n"){                 // tal vez el salto de linea podria ser diferente
            ...
            if(read_line.rfind("<",0)){
                vector<string> temp_read_line = partition(read_line,",");
                string temp_line = lstrip(temp_read_line[0], "<");
                ...
                temp_read_line = partition(read_line, ":");
            ...
            if(read_line.rfind("grbl",0)){
                ...
                temp_decode = partition(read_line, " ");
                temp_decode = partition(temp_decode[2], "f ");
                if(temp_decode[1] == "k "){
...
void SerialGRBL::SerialGotoXY(float temp_x, float temp_y, string temp_f){
...
void SerialGRBL::SerialGotoHome(string temp_f){
...
void SerialGRBL::NewLayer(double temp_thickness, int temp_override_build){
    ...
    if(this->homed_state == 1){
        ...
        this->SerialGotoXY(this->nl_back_pos_x, this->nl_back_pos_y, to_string(this->nl_travel_speed));   // OJO con esto el string
        ...
        this->SerialWriteBufferRaw("G90");
        this->SpreaderSet(1);
        this->SerialGotoXY(this->nl_front_pos_x, this->nl_back_pos_y, to_string(this->nl_feed_speed));
        this->SpreaderSet(0);
        this->SerialWriteBufferRaw("G91");
        this->SerialWriteBufferRaw("G1 Z" + to_string(this->nl_piston_hysteresis) + " F" + to_string(this->nl_piston_speed));
